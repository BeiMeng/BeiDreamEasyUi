		...
		enableDnd: function(jq, id){
			...
			return jq.each(function(){
				...
				function append(sRow, dRow) {
				    ...
				    setTimeout(function() {
				        var row = $.easyui.treegrid.moveToChild(t, sRow[opts.idField], dRow[opts.idField]);
				        opts.onDrop.call(target, dRow, row, 'append');
				    }, 100);
				}
				function insert(sRow, dRow, point) {
				    if (!sRow || !dRow)
				        return;
				    var row;
				    if (sRow.SortId > dRow.SortId)
				        row = $.easyui.treegrid.moveToBefore(t, sRow[opts.idField], dRow[opts.idField]);
				    else
				        row = $.easyui.treegrid.moveToAfter(t, sRow[opts.idField], dRow[opts.idField]);
					opts.onDrop.call(target, dRow, row, point);
				}
			});
		}
	});
